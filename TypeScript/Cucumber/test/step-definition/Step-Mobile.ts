...
Given(/^user is on HUGO BOSS mobile website/, async () => {
...
When(/^guest user adds an item to the shopping bag on mobile website$/, async () => { 
...
Then(/^guest user should be able to proceed to payment checkout on mobile website$/, async () => { 
...
When(/^guest user creates an account on mobile website$/, async () => { 
	const buttonMenu = $('button[class="main-header__menu-btn js-main-nav-toggle"]');
	await checkElementExist(buttonMenu);
	await buttonMenu.click();
	await browser.pause(2000);
	
	const menuAccount = $('use[*|href="#icon_customer"]');
...
Then(/^user should see logout button on mobile website$/, async () => { 
